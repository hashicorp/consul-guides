...
1.) Use Wetty (Web + tty), a web terminal for the cluster over HTTP and HTTPS
  ...
  ${join("\n  ", formatlist("%s Wetty Url: http://%s:3030/wetty", list("Bastion", "Consul"), list(element(concat(module.network_aws.bastion_ips_public, list("")), 0), module.consul_aws.consul_app_lb_dns)))}
...
If public, you can SSH into the Consul nodes directly through the LB.
  ...
  $ ${format("ssh -A -i %s \\\n      %s@%s", module.network_aws.private_key_filename, module.consul_aws.consul_username, module.consul_aws.consul_network_lb_dns)}
